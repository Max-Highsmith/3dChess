...
public class Bishop extends Piece
{
       ...
       public Bishop(int x, int y, int z, Tribe t )
    {
        t_type = Piece.Token_Type.Bishop;
        coordinates.set_Coordinates(x,y,z);
        color = t;
        coordinateArray = new CoordinateArray(12,4);
      ...
      Outer_Loop: while (d<12)
      {
          for(int x=-1; x<2; x++)
          {
              for(int y=-1; y<2 ;y++)
              {
                  for(int z=-1; z<2; z++)
                  {
                      if (d > 25) { break Outer_Loop;}
                      if(Math.abs(x)+Math.abs(y)+Math.abs(z) ==2 )         // gurantees only 2d moves pass
                      {
