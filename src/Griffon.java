...
public class Griffon extends Piece
{
  
    public Griffon(int x, int y, int z, Tribe t )
    {
        t_type = Piece.Token_Type.Griffon;
        coordinates.set_Coordinates(x,y,z);
        color = t;
        coordinateArray = new CoordinateArray(8,4);
      ...
      Outer_Loop: while (d<8)
      {
          for(int x=-1; x<2; x++)
          {
              for(int y=-1; y<2 ;y++)
              {
                  for(int z=-1; z<2; z++)
                  {
                      if (d > 8) { break Outer_Loop;}
                      if(Math.abs(x)+Math.abs(y)+Math.abs(z) ==3 )         // gurantees only 2d moves pass
                      {
